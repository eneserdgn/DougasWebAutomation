package stepDefinitions;

import io.cucumber.java.en.Given;
import io.cucumber.java.en.When;

public class ProductsPage {

    @When("select on {string} Marke on Products Page")
    public void selectOnMarkeOnProductsPage(String arg0) {
    }

    @When("select on {string} Producart on Products Page")
    public void selectOnProducartOnProductsPage(String arg0) {
    }

    @When("select on {string} Geschenk fur on Products Page")
    public void selectOnGeschenkFurOnProductsPage(String arg0) {
    }

    @When("select on {string} Fur Wen on Products Page")
    public void selectOnFurWenOnProductsPage(String arg0) {
    }
}
